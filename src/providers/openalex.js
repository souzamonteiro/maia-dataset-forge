...
export async function searchOpenAlex({
  ...
  return j.results.map((w) => ({
    pdfLocations: (w.locations || []).filter(l => l.is_oa && l.pdf_url).map(l => ({url:l.pdf_url, license:l.license, landingUrl:l.landing_page_url})),
    provider: "openalex",
    id: w.id?.split("/").pop(),
    ...
    abstract: invAbs(w.abstract_inverted_index),
